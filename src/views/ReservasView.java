...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		JLabel lblValor = new JLabel("PRECIO");
		...
		txtFormaPago.setModel(new DefaultComboBoxModel(new String[] {"T Credito", "T Debito", "Efectivo"}));
		...
		btnsiguiente.setBounds(209, 486, 151, 42);
		...
		lblSiguiente.setFont(new Font("Dialog", Font.PLAIN, 14));
		lblSiguiente.setBounds(0, 0, 141, 35);
		btnsiguiente.add(lblSiguiente);
		lblSiguiente.setForeground(Color.BLACK);
		lblSiguiente.setFont(new Font("Roboto", Font.PLAIN, 25));
	...
	private void guardarReserva() {
		if(txtFechaEntrada.getDate() != null && txtFechaSalida.getDate() != null && !txtValor.equals("") && !txtFormaPago.getSelectedItem().toString().equals("")) {
			...
			JOptionPane.showMessageDialog(this, "Debes rellenar todas las casillas ");
		}
	}
	...
	public void calcularValor(JDateChooser dataE, JDateChooser dataS) {
		if(dataE.getDate()!=null && dataS.getDate()!=null) {
			if(dataE.getDate().after(dataS.getDate())) {
				JOptionPane.showMessageDialog(null, "La fecha de entrada es incorrrecta" , "Error en fechas", JOptionPane.ERROR_MESSAGE);
				return;
			}
			 Calendar inicio = dataE.getCalendar();
			 Calendar fin = dataS.getCalendar();
			 int dias = -1; //esto para que el sistema cuenta para cobro a partir del dia siguiente
			 int noche = 40;
			 int valor;
